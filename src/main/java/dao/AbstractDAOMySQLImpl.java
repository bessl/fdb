...
public abstract class AbstractDAOMySQLImpl<T extends ModelBase> implements
		BaseDAO<T> {
	...
	public int save(Connection connection, T entity) {
		...
		return entity.isNew() ? insert(connection, entity) : update(connection,
				entity);
	}
		...
		try {
			if (connection.isClosed() || !connection.isValid(250)) {
				throw new DAOException(
						"Connection ist geschlossen oder ungueltig!");
			}
		} catch (SQLException e) {
			throw new DAOException(
					"Connection kann nicht ueberprueft werden!", e);
		}
	}
